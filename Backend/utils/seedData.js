...
async function seedStudentData(studentId, currentSem = 4, conn = null) {
    const executor = conn || db;
    try {
        const [existingSemesters] = await executor.execute('SELECT id FROM semesters WHERE student_id = ?', [studentId]);
        ...
        for (let s = 1; s <= targetSem; s++) {
            ...
            const [semResult] = await executor.execute(
            ...
            // To speed up, we'll collect some data for bulk processing if needed, 
            // but for now let's just use the transactional executor correctly.
            for (let i = 0; i < selectedSubjects.length; i++) {
                ...
                const [subResult] = await executor.execute(
                ...
                // Random Attendance (reduced to 10 entries to speed up registration)
